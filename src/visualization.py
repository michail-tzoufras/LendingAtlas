...
def rank_by_status(df, feature):
    ...
    Percentage = np.array(
        [d * 100.0 / float(p + d) for p, d in zip(Number_Paid, Number_Defaults)]
    )
...
def make_tiers(df, c, number_tiers):
    ...
    tier_length = int(
        (last - i + 1) / (number_tiers - int(len(last_tier) > 0) - int(len(tier) > 0))
    )
...
def find_similar(name, output_path, weights, index, rindex, n=10):
    ...
    data["distance"].plot.barh(
        color=colors, alpha=0.9, figsize=(10, 8), edgecolor="k", linewidth=2
    )
...
class Make_Visualizations(object):
    ...
    def country_vs_status(self, df):
        ...
        plt.xticks(
            range(Countries.shape[0]), Countries, rotation="vertical", fontsize=12
        )
    ...
    def data_exploration(self, df):
        ...
        pd.crosstab(df["Sector"], df["Status"]).plot(
            kind="bar", figsize=(15, 6), cmap="Set1"
        )
...
class Visualize_Embeddings(object):
    def __init__(self, embeddings_path, output_path, df, clms):
    ...
    def plot_embeddings(
        self, feature, filename, output_filename, interesting_values, tiers=[]
    ):
    ...
    def plot_town_embedding(
        ...
        for j, tier in enumerate(tiers):
            ...
            for i, txt in enumerate(labels):
                if txt in tier:
                    if txt not in interesting_values:
                        xaxis.append(weights[i, 0])
                        yaxis.append(weights[i, 1])
                        ax1.scatter(
                            xaxis, yaxis, marker="o", color="lightblue", alpha=0.5, s=80
                        )
                ...
                if txt in tier:
                    if txt in interesting_values:
                        if text_labels:
                            ax1.annotate(
                                txt,
                                (weights[i, 0] + 0.5, weights[i, 1] + 0.5),
                                fontsize=16,
                            )
                        ax1.scatter(
                            weights[i, 0],
        ...
        fig1.savefig(self.output_path + output_filename, dpi=300)
    ...
    def plot_embeddings_similarity(self, feature, value):
        ...
        weights, labels = self.weights_n_labels(
            self.embeddings_path + feature + "_embedding.csv"
        )
    ...
    def display(self, feature, interesting_values=[]):
        self.plot_embeddings(
            feature,
            feature + "_embedding.csv",
            feature + "_embedding_plot.png",
            interesting_values,
            self.feature_tiers[feature],
        )

    def display_towns(
        self, feature, highlighted_country, interesting_values, text_labels=True
    ):
        self.plot_town_embedding(
            feature,
            feature + "_embedding.csv",
            highlighted_country + "_town_embedding_plot.png",
            highlighted_country,
            interesting_values,
            text_labels,
            self.feature_tiers[feature],
