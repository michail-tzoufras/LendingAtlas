...
def combine_rare(df_in, column, output_path, Limit=100):
    """Combine rare categorical data to Rare_"""
    df_out = df_in.copy()
    df_out[column] = "Rare_" + column
    i = 0
    while df_in[column].value_counts().iloc[i] > Limit:
        name = df_in[column].value_counts().index[i]
        (df_out[column]).loc[df_in["Town"] == name] = name

    df_out.to_csv(output_path + "df_Rare_" + column + ".csv", mode="w", index=False)
