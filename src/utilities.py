...
def new_csv_writer(path, name, filenumber, headers, delimiter):
    ...
    writer = csv.writer(
        open(os.path.join(path, name % filenumber), "w"), delimiter=delimiter
    )
...
def split(
    filehandler, output_path, output_name_template, row_limit=100000, delimiter=","
):
...
def attach_partner_id(
    file1="/Users/mtzoufras/Desktop/Insight/Insight_Project_Data/kiva_ds_csv/loans.csv",
